...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (exportSummaryCsv) {
        exportSummaryCsv.addEventListener('click', function() {
            showLoading('Exportando resumen CSV...');
            
            fetch(`/api/export/${taskId}/csv`)
                .then(response => {
                    if (!response.ok) {
                        throw new Error('Error al exportar el resumen CSV');
                    }
                    return response.json();
                })
                .then(data => {
    ...
    if (exportAllJson) {
        exportAllJson.addEventListener('click', function() {
            showLoading('Exportando todos los informes JSON...');
            
            fetch(`/api/export-all/${taskId}/json`)
                .then(response => {
                    if (!response.ok) {
                        throw new Error('Error al exportar todos los informes JSON');
                    }
                    return response.json();
                })
                .then(data => {
    ...
    // Añadir botones para exportar todos los informes en HTML y CSV
    const exportOptionsDiv = document.querySelector('.export-options');
    
    if (exportOptionsDiv) {
        // Botón para exportar todos los informes HTML
        const exportAllHtml = document.createElement('button');
        exportAllHtml.id = 'export-all-html';
        exportAllHtml.className = 'btn btn-secondary';
        exportAllHtml.innerHTML = '<i class="fas fa-file-alt"></i> Exportar Todos los Informes HTML';
        exportOptionsDiv.appendChild(exportAllHtml);
        
        // Botón para exportar todos los informes CSV
        const exportAllCsv = document.createElement('button');
        exportAllCsv.id = 'export-all-csv';
        exportAllCsv.className = 'btn btn-secondary';
        exportAllCsv.innerHTML = '<i class="fas fa-file-csv"></i> Exportar Todos los Informes CSV';
        exportOptionsDiv.appendChild(exportAllCsv);
        
        // Añadir eventos a los nuevos botones
        exportAllHtml.addEventListener('click', function() {
            showLoading('Exportando todos los informes HTML...');
            
            fetch(`/api/export-all/${taskId}/html`)
                .then(response => {
                    if (!response.ok) {
                        throw new Error('Error al exportar todos los informes HTML');
                    }
                    return response.json();
                })
                .then(data => {
                    if (data.file) {
                        ...
                        throw new Error('No se pudo generar el archivo HTML');
                    }
                })
                .catch(error => {
                    console.error('Error:', error);
        ...
        exportAllCsv.addEventListener('click', function() {
            showLoading('Exportando todos los informes CSV...');
            
            fetch(`/api/export-all/${taskId}/csv`)
                .then(response => {
                    if (!response.ok) {
                        throw new Error('Error al exportar todos los informes CSV');
                    }
                    return response.json();
                })
                .then(data => {
